...
| 2026-07-16 | **Safety filtering is required, not deferred.** Real ORS routes put the runner on `highway=trunk` — a known ORS foot-walking limitation (issue #1212), compounded by patchy North-American sidewalk tagging in OSM. Fix: request `extra_info: ["waytype"]` + the `quiet` factor, then **hard-reject waytype 1** (primary/motorway/trunk), **penalise waytype 2** (secondary/tertiary/unclassified — too lumpy to reject), **prefer 7/3/4/6** (footway/street/path/cycleway). Missing waytype data = unsafe (fail closed). All candidates unsafe → `NoSafeRouteError`, not a dangerous route. |
| 2026-07-16 | A filter **reduces** risk, it cannot **guarantee** a safe route (a sidewalk-less residential street is invisible in OSM data). So reviewing the route on a map before running is a **product requirement**, not a nicety. |
| 2026-07-16 | ORS's `quiet`/`green` foot weightings are **rejected by the public API** (engine 9.9.0 → 400 code 2002) — they need optional data layers the hosted instance wasn't built with. Dropped. `extra_info` top-level is fine. Consequence: no up-front biasing, so the waytype hard-reject does all the safety work and `N` may need raising once we see the rejection rate. Real biasing needs the self-hosted engine. |
| 2026-07-16 | **ORS `round_trip` length is a loose suggestion, not a target** — a 5 km request returned 4787 / 5668 / 7269 / **10129** m across four seeds. So: candidates outside **±10% of target** (`DISTANCE_TOLERANCE`) are discarded outright, and selection ranks **safety → closest → prefer overshoot**. The band alone eliminates the arterial-heavy routes, since they tend to be the long ones. |
| 2026-07-16 | **No early-out; all seeds fetched in parallel.** Stopping at the first acceptable candidate leaves the safety ranking nothing to compare. ~10 calls per cache miss, roughly one request in wall-clock. Seeds are **injected** (`DEFAULT_SEEDS`, fixed) so runs are reproducible — never randomise at module load, or a bad route can't be replayed. |
| 2026-07-16 | `alternative_routes` **cannot be combined with `round_trip`** (it needs a start and an end; round_trip takes one coordinate). Confirmed by testing — so multiple candidates always means multiple API calls. |
| 2026-07-16 | **ORS overshoots `round_trip.length` by a consistent ~1.30×.** Measured over 40 seeds at two distances: requesting 3760 m returned a 4899 m median against a 5000 m target (1.303×); requesting 7500 m returned 9713 m against 10000 m (1.295×). So we request `target × REQUEST_SCALE (0.77)` and judge the tolerance band against the **real target**. In-band hit rate 15% → 35–43%, which drops the chance of ten seeds finding nothing from ~20% to ~1%. |
| 2026-07-16 | Sweep data also showed **39–40 of 40 candidates pass the waytype-1 safety reject** — the hard reject is a rarely-triggered floor. The real protection comes from the tolerance band (long routes skew arterial) and the safety *ranking*. Keep the reject, but don't mistake it for what's doing the work. |
| 2026-07-16 | **The distance band is asymmetric: target → target+10%.** Short routes are rejected by default — finishing a "5k" at 4.8 km is the failure a runner notices; the overshoot is cooldown. `allowShorter` is a **per-runner setting** (default off) that opens the lower side and roughly doubles the candidate pool. Should surface as a UI setting and a query param. |
| 2026-07-16 | **Safety is compared in buckets (`SAFETY_GRANULARITY`), not as raw floats.** A continuous score always wins outright, which silently kills every ranking key beneath it — a mistake made twice (safety-as-tiebreak, then overshoot-as-tiebreak). Bucketing lets a clearly safer route win while similar routes are settled on distance. |
